...
def make_distribution_chart(prefix: str, title_suffix: str) -> None:
    ...
    bars = ax.bar(
        [labels[item] for item in df["sentiment"]],
        df["count"],
        color=[colors[item] for item in df["sentiment"]],
    )
...
def make_by_source_chart(prefix: str, title_suffix: str) -> None:
    df = pd.read_csv(RESULTS_DIR / f"{prefix}step2_sentiment_by_source.csv")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create sentiment charts from report CSV files")
    ...
    print("Created sentiment charts.")
